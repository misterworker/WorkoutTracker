...
class SetAdapter(
...
) : ListAdapter<Triple<Float, Int, String>, SetAdapter.SetViewHolder>(SetDiffCallback()) {
    ...
    override fun onBindViewHolder(holder: SetViewHolder, position: Int) {
        ...
        if (completions[position]) {
            holder.weightInput.setText(weight.toString())
            holder.repsInput.setText(reps.toString())
            val color = ContextCompat.getColor(holder.itemView.context, R.color.special_date_background)
        ...
        // Temporarily remove the listener while setting the checkbox state
        holder.setCheckbox.setOnCheckedChangeListener(null)

        // Set the checkbox state based on the completion array
        ...
        holder.setCheckbox.setOnCheckedChangeListener { _, isChecked ->
            val pos = holder.bindingAdapterPosition
            if (pos != RecyclerView.NO_POSITION) {
                completions[pos] = isChecked
                listener.onSetCompletionChanged(pos, isChecked)
                if (isChecked) {
                    val color = ContextCompat.getColor(holder.itemView.context, R.color.special_date_background)
                    val colorFilter = BlendModeColorFilter(color, BlendMode.SRC_IN)
                    weightDrawable?.colorFilter = colorFilter
                    repsDrawable?.colorFilter = colorFilter
                    val currentWeight = holder.weightInput.text.toString().toFloatOrNull() ?: weight
                    val currentReps = holder.repsInput.text.toString().toIntOrNull() ?: reps
                    holder.weightInput.setText(currentWeight.toString())
                    holder.repsInput.setText(currentReps.toString())
                } else {
        ...
        holder.swipeLayout.setOnActionsListener(object : SwipeLayout.SwipeActionsListener {
            ...
            override fun onClose() {
                if (holder.setCheckbox.isChecked) {
                    // Restore green background if checkbox is checked
                    val greenColor = ContextCompat.getColor(holder.itemView.context, R.color.special_date_background)
                    val greenColorFilter = BlendModeColorFilter(greenColor, BlendMode.SRC_IN)
                    weightDrawable?.colorFilter = greenColorFilter
                    repsDrawable?.colorFilter = greenColorFilter
                } else {
                    // Clear color filter if checkbox is not checked
                    weightDrawable?.clearColorFilter()
                    repsDrawable?.clearColorFilter()
                }
